...
create or replace NONEDITIONABLE PROCEDURE PRO_RECORD_MMDD_DAWN(P_MEM_ID record.record_mem_id%TYPE, P_SUBJECT_NAME subject.subject_name%TYPE, P_MONTH NUMBER, P_DAY NUMBER)
IS
        V_SEQ_RECORD_ID NUMBER;
         V_I_NUMBER NUMBER;
         P_MAXCOUNT CONSTANT NUMBER := 8;
BEGIN
    -- 다음 시퀀스 값을 선택합니다.

    for I in 4..P_MAXCOUNT loop
        V_I_NUMBER := I;
...
create or replace NONEDITIONABLE PROCEDURE PRO_RECORD_MMDD_NIGHT(P_MEM_ID record.record_mem_id%TYPE, P_SUBJECT_NAME subject.subject_name%TYPE, P_MONTH NUMBER, P_DAY NUMBER)
IS
        V_SEQ_RECORD_ID NUMBER;
         V_I_NUMBER NUMBER;
         P_MAXCOUNT CONSTANT NUMBER := 3;
BEGIN
    -- 다음 시퀀스 값을 선택합니다.
