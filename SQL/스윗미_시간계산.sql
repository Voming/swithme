...
--
(
SELECT RECORD_START,RECORD_SUBJECT_ID, SUBSTR(NUMTODSINTERVAL( SUM( CAST(RECORD_END as DATE) - CAST(RECORD_START as DATE) ), 'day' ), 12, 8) as DIFFTIME
FROM RECORD 
WHERE RECORD_MEM_ID = 'won'
            AND RECORD_START >= SYSDATE - 30 -- 30일 이전부터의 레코드를 고려
        GROUP BY
             RECORD_START,RECORD_SUBJECT_ID --,to_char(RECORD_START, 'yyyy-mm-dd')
) t2
--
on (SUBJECT_ID = RECORD_SUBJECT_ID)
